...
$resultc = $db->prepare("SELECT * FROM appointments where status='Open' and pangea_userid=:pangea_userid ");
...
$resultc = $db->prepare("SELECT * FROM appointments where status='Closed' and pangea_userid=:pangea_userid ");
$resultc->execute(array(':pangea_userid' =>$userid_sess));
$rowsc = $resultc->fetch();
