...
$pui = strip_tags($_GET['pangea_userid']);
...
$uid = $json['result']['id'];
$fullname = $json['result']['profile']['fullname'];
$profession = $json['result']['profile']['profession'];


$last_login_at= $json['result']['last_login_at'];
$created_at = $json['result']['created_at'];
$login_count = $json['result']['login_count'];
...
<h3> <?php echo $fullname; ?>'s Medical Profile</h3>
<b>Pangea UserId: </b><?php echo htmlentities(htmlentities($uid, ENT_QUOTES, "UTF-8")); ?> <br>
<b>Name: </b><?php echo htmlentities(htmlentities($fullname, ENT_QUOTES, "UTF-8")); ?> <br>
...
$result->execute(array(':pangea_userid'  =>$pui));
$rows = $result->fetch();
...
$resultc->execute(array(':pangea_userid'  =>$pui));
$rowsc = $resultc->fetch();
...
$('.loader_x').fadeIn(400).html('<br><div style="background:#ccc;color:black; width:100%;height:30%;text-align:center"><img src="ajax-loader.gif">&nbsp;Please Wait, Your Data is being Loaded</div>');
 ...
 var bck = $('#bc').DataTable({
  ...
  "order":[],
  "ajax":{
   url:"./backend/appointments_patients.php",
   type:"POST",
   data:{get_content:get_content, backup_type:backup_type, pui:pui}
  },
  "columnDefs":[
   {
    "orderable":false,
